...
function isTitle(cm, line, level) {
    ...
    if (level !== undefined && level !== reference_level) {
        return null;
    }
...
function rearrange_list(cm, line) {
    ...
    function find_limit_inferior(_cm, _line) {
        let content; let p; let match; let line_candidate = _line;
        do {
            content = _cm.getLine(_line);
...
function swap(cm, from, to) {
    ...
    const to_content = cm.getRange({ line: to[0], ch: 0 }, { line: to[1], ch: cm.getLine(to[1]).length });
    const cursor = cm.getCursor();
