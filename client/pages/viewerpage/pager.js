...
export class Pager extends React.Component {
        ...
        if (this.state.files[n]) {
            ...
            const preload_index =
                ...
                (this.state.n === this.state.files.length - 1 && n === 0)
                    ? this.calculateNextPageNumber(n)
                    : this.calculatePrevPageNumber(n);
    ...
    setNavigation(props) {
        Files._ls_from_cache(dirname(props.path))
            ...
            .then((f) =>
                f.results.filter(
                    (file) =>
                        (isImage(file.name) || isVideo(file.name)) && file.type === "file",
                ),
            )
    ...
    render() {
        const inputWidth =
            this.state.n === undefined ? 12 : (this.state.n + 1).toString().length * 12;
        const nextLink = () => {
            const l =
                this.state.files[this.calculateNextPageNumber(this.state.n)];
            return ((l && l.link) || "") + window.location.search || "#";
        };
        const prevLink = () => {
            const l =
                this.state.files[this.calculatePrevPageNumber(this.state.n)];
            return ((l && l.link) || "") + window.location.search || "#";
        };
        const current_page_number =
            this.state.n === undefined ? "" : this.state.n + 1;
        return (
            <div className="component_pager">
                <div className="wrapper no-select">
                    <NgIf cond={this.state.files.length > 0} type="inline">
                        <Link to={prevLink()}>
                            <Icon name="arrow_left_white" />
                        </Link>
